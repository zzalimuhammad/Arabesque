...
class Miniterm(object):
    ...
    def file_dialog(self, initial):
        if platform.system() == 'Darwin':
            ...
            try:
                ...
                destname = raw_input("File on Arabesque: ")
                return pathname, destname
            else:
                pathname = filedialog.askopenfilename(title="File to Upload", initialfile=initial, filetypes=[("Arabesque Config", "*.yaml *.flnc *.txt"), ("All files", "*")])
                print("path",pathname)
    ...
    def reset_fluidnc(self):
        """Pulse the reset line for Arabesque"""
    ...
    def send_xmodem(self, filename, destname):
        with self.console:
            try:
                ...
                # send the command to put Arabesque in receive mode
                self.serial.write(
